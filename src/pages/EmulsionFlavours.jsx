...
export default function EmulsionFlavours() {
  ...
  const filteredProducts = useMemo(() => {
    return EMULSION_FLAVOURS.filter((item) => {
      // 1. Profile family filter
      const matchesFilter =
  ...
  return (
    <div className="bg-white min-h-screen text-slate-900 pt-20 sm:pt-24">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 pb-12">
        {filteredProducts.length > 0 ? (
          ...
          <div className="py-16 text-center bg-slate-50 rounded-3xl border border-slate-200">
            ...
            <h3 className="text-lg font-bold text-slate-900">
              No matching emulsion flavours found
            </h3>
            <p className="text-xs text-slate-500 mt-1 max-w-sm mx-auto">
              We couldn't find any emulsion matching "{searchQuery}". Try searching by aroma profile, or clear filters.
            </p>
            <button
              type="button"
              onClick={() => {
        ...
        {/* Expandable Custom Emulsion Formulations Section */}
        <div className="mt-14 pt-10 border-t border-slate-200 text-center">
          ...
          >
            <span>{showSpecialtyTable ? 'Hide' : 'Explore'} Additional Customized Emulsion Flavour Formulations</span>
            <span className="text-xs">{showSpecialtyTable ? '▲' : '▼'}</span>
          </button>

          {showSpecialtyTable && (
            <div className="mt-6 text-left max-w-4xl mx-auto rounded-2xl border border-slate-200 overflow-hidden shadow-sm animate-fadeIn">
              <div className="bg-[#d9edf7] p-3 border-b border-slate-200">
                <h4 className="font-black text-slate-900 text-sm text-center">
                  Beverage Clouding &amp; Emulsion Formulations (Customized Weighting Agents)
                </h4>
              </div>
              <div className="p-4 bg-white grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2 text-xs">
                {SPECIALTY_EMULSION_PROFILES.map((profile, idx) => {
                  let bg = 'bg-slate-50'
