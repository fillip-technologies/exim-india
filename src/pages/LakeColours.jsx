...
export default function LakeColours() {
  const [activeFilter, setActiveFilter] = useState('all')
  const [searchQuery, setSearchQuery] = useState('')

  // Filtered and searched product list
  const filteredProducts = useMemo(() => {
    return LAKE_COLORS.filter((item) => {
        ...
        {filteredProducts.length > 0 ? (
          ...
          <div className="py-16 text-center bg-slate-50 rounded-3xl border border-slate-200">
            ...
            <h3 className="text-lg font-bold text-slate-900">
              No matching lake colours found
            </h3>
            <p className="text-xs text-slate-500 mt-1 max-w-sm mx-auto">
              We couldn't find any products matching "{searchQuery}". Try searching by lake name, E number, C.I. number, or clear filters.
            </p>
            <button
              type="button"
              onClick={() => {
