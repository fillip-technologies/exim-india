...
export default function NaturalFruitPowder() {
  ...
  const filteredProducts = useMemo(() => {
    return NATURAL_FRUIT_POWDERS.filter((item) => {
      ...
      const matchesQuery =
        ...
        item.fruitFamily.toLowerCase().includes(q) ||
  ...
  return (
    <div className="bg-white min-h-screen text-slate-900 pt-20 sm:pt-24">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 pb-12">
        {filteredProducts.length > 0 ? (
          ...
          <div className="py-16 text-center bg-slate-50 rounded-3xl border border-slate-200">
            ...
            <h3 className="text-lg font-bold text-slate-900">
              No matching fruit powders found
            </h3>
            <p className="text-xs text-slate-500 mt-1 max-w-sm mx-auto">
              Try adjusting your search query or selecting &quot;All Fruit Powders&quot;.
            </p>
            <button
              onClick={() => {
                setActiveFilter('all')
        ...
        {/* ============================================================== */}
        {/* EXPANDABLE SECTION: SPRAY-DRYING TECHNOLOGY & APPLICATION GUIDE */}
        {/* ============================================================== */}
        <div className="mt-14 pt-8 border-t border-slate-200">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-slate-50 p-6 rounded-2xl border border-slate-200">
            <div>
              <span className="text-[11px] font-extrabold uppercase tracking-wider text-emerald-800 bg-emerald-100/70 px-2.5 py-0.5 rounded-md border border-emerald-200">
                Spray-Drying Formulation Guide
              </span>
              <h3 className="text-lg sm:text-xl font-bold font-heading text-slate-900 mt-1.5">
                Low-Temperature Spray Drying, Flavor Encapsulation &amp; Reconstitution Manual
              </h3>
              <p className="text-xs sm:text-sm text-slate-600 mt-0.5">
                Technical processing standards across beverage premixes, bakery doughs, confectionery fillings, and nutraceutical tablets.
              </p>
            </div>
            <button
          ...
          {showTechGuide && (
            <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4 animate-fadeIn">
              {FRUIT_POWDER_APPLICATIONS.map((app) => (
                <div
                  key={app.category}
                ...
                >
                  ...
                  <div className="space-y-2 text-xs text-slate-600">
                    ...
                    <div className="pt-2 border-t border-slate-100 text-emerald-900 font-medium bg-emerald-50/50 p-2 rounded-lg">
                      <strong>Processing Advantage: </strong>
                      {app.advantage}
                    </div>
                  </div>
                </div>
