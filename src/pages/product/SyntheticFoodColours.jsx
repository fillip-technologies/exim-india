...
export default function SyntheticFoodColours() {
  const [activeFilter, setActiveFilter] = useState('all')
  const [searchQuery, setSearchQuery] = useState('')

  // Filtered and searched product list
  const filteredProducts = useMemo(() => {
    return SYNTHETIC_FOOD_COLORS.filter((item) => {
      // 1. Color family filter
      const matchesFilter =
        activeFilter === 'all' ||
        (activeFilter === 'red' && item.colorFamily === 'red') ||
        (activeFilter === 'yellow' && item.colorFamily === 'yellow') ||
        (activeFilter === 'blue' && (item.colorFamily === 'blue' || item.colorFamily === 'green')) ||
        (activeFilter === 'brown' && (item.colorFamily === 'brown' || item.colorFamily === 'black'))
      ...
      const matchesQuery =
        ...
        item.colorNo.toLowerCase().includes(q) ||
        ...
        {filteredProducts.length > 0 ? (
          ...
          <div className="py-16 text-center bg-slate-50 rounded-3xl border border-slate-200">
            ...
            <h3 className="text-lg font-bold text-slate-900">
              No matching synthetic food colours found
            </h3>
            <p className="text-xs text-slate-500 mt-1 max-w-sm mx-auto">
              We couldn't find any products matching "{searchQuery}". Try searching by color name, C.I. number, or clear filters.
            </p>
            <button
              type="button"
              onClick={() => {
